...
/**
 * DateHelper class that 7 Carbon functionality for various date comparisons and conversions.
 */
class DateHelper
{
    /**
    ...
    public static function getFutureDateTime($value, $unit = "minutes")
    {
        $currentDateTime = new \DateTime(); // Get the current date/time
    
        // Use a switch case to modify the date/time based on the unit
        switch ($unit) {
            case "minutes":
                $currentDateTime->modify("+$value minutes");
                break;
            case "days":
                $currentDateTime->modify("+$value days");
                break;
            case "months":
                $currentDateTime->modify("+$value months");
                break;
            case "years":
                $currentDateTime->modify("+$value years");
                break;
            case "weeks":
                $currentDateTime->modify("+$value weeks");
                break;
            default:
                // Handle invalid units by throwing an exception
                throw new \InvalidArgumentException("Invalid unit provided for date modification: $unit");
        }
    
        return $currentDateTime; // Return the updated date/time
    }
